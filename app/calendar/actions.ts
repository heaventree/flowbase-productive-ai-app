...
import { db, users, calendarItems } from "@/db";
...
export async function listCalendarItems() {
  ...
  const items = await db.select().from(calendarItems).where(eq(calendarItems.userId, userId));
...
export async function createCalendarItem(input: CalendarItemInput, asDraft = false) {
  ...
  const result = await db.insert(calendarItems)
    .values({
      userId,
      title,
  ...
  if (!result[0]) throw new Error("Failed to create calendar item.");
...
export async function updateCalendarItem(id: number, input: CalendarItemInput, asDraft = false) {
  ...
  const result = await db.update(calendarItems)
    .set({
      title,
      description: cleanOptionalText(input.description),
      itemType: normalizeType(input.itemType),
...
export async function scheduleCalendarItem(id: number, scheduledDate: string) {
  ...
  const result = await db.update(calendarItems)
    .set({ scheduledDate: date, isDraft: false, updatedAt: new Date().toISOString() })
    .where(and(eq(calendarItems.id, id), eq(calendarItems.userId, userId)))
    .returning();
...
export async function moveCalendarItemToDraft(id: number) {
  ...
  const result = await db.update(calendarItems)
    .set({ scheduledDate: null, isDraft: true, updatedAt: new Date().toISOString() })
    .where(and(eq(calendarItems.id, id), eq(calendarItems.userId, userId)))
    .returning();
...
export async function deleteCalendarItem(id: number) {
  ...
  const result = await db.delete(calendarItems)
    .where(and(eq(calendarItems.id, id), eq(calendarItems.userId, userId)))
